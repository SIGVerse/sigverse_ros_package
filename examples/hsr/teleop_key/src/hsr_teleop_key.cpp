...
//  void suction_result_callback(const tmc_suction::SuctionControlActionResult::ConstPtr& suction_result);
...
SIGVerseHsrTeleopKey::SIGVerseHsrTeleopKey()
{
  ...
  sub_msg_         = node_->create_subscription<std_msgs::msg::String>       ("/hsrb/message/to_robot", 10, std::bind(&SIGVerseHsrTeleopKey::message_callback,    this, std::placeholders::_1));
  sub_joint_state_ = node_->create_subscription<sensor_msgs::msg::JointState>("/hsrb/joint_states",     10, std::bind(&SIGVerseHsrTeleopKey::joint_state_callback, this, std::placeholders::_1));
...
int SIGVerseHsrTeleopKey::can_receive( int fd )
{
  fd_set fdset;
...
void SIGVerseHsrTeleopKey::message_callback(const std_msgs::msg::String::SharedPtr message)
{
  RCLCPP_INFO(node_->get_logger(), "Subscribe message: %s", message->data.c_str());
}

void SIGVerseHsrTeleopKey::joint_state_callback(const sensor_msgs::msg::JointState::SharedPtr joint_state)
{
...
void SIGVerseHsrTeleopKey::send_message(const std::string &message)
{
...
void SIGVerseHsrTeleopKey::move_base_twist(const double linear_x, const double linear_y, const double angular_z)
{
...
void SIGVerseHsrTeleopKey::move_base_joint_trajectory(const double linear_x, const double linear_y, const double theta, const double duration_sec)
{
...
void SIGVerseHsrTeleopKey::operate_arm(const double arm_lift_pos, const double arm_flex_pos, const double arm_roll_pos, const double wrist_flex_pos, const double wrist_roll_pos, const double duration_sec)
{
  trajectory_msgs::msg::JointTrajectory joint_trajectory;
...
void SIGVerseHsrTeleopKey::operate_arm(const std::string &name, const double position, const double duration_sec)
{
...
void SIGVerseHsrTeleopKey::operate_arm_flex(const double arm_flex_pos, const double wrist_flex_pos)
{
...
double SIGVerseHsrTeleopKey::get_duration_rot(const double next_pos, const double current_pos)
{
  return std::max<double>((std::abs(next_pos - current_pos) * 1.2), 1.0);
}

void SIGVerseHsrTeleopKey::operate_hand(const bool is_hand_open)
{
...
//void SIGVerseHsrTeleopKey::send_suction_goal(const bool &sution_on)
//{
...
//void SIGVerseHsrTeleopKey::suction_result_callback(const tmc_suction::SuctionControlActionResult::ConstPtr& suction_result) 
//{
//  // 3: SUCCEEDED
//  // 4: ABORTED(Timeout)
...
void SIGVerseHsrTeleopKey::show_help()
{
    ...
    if(can_receive(kfd))
    {
      ...
      switch(c)
      {
        ...
        case 'u':
        {
          ...
          move_base_joint_trajectory(+1.0, +1.0, +M_PI_4, 10);
          break;
        }
        case 'i':
        {
          RCLCPP_DEBUG(logger, "Move Forward");
          move_base_joint_trajectory(+1.0, 0.0, 0.0, 10);
          break;
        }
        case 'o':
        {
        ...
        case 'j':
        {
          ...
          move_base_joint_trajectory(0.0, +1.0, +M_PI_2, 10);
          break;
        }
        case 'k':
        {
          RCLCPP_DEBUG(logger, "Stop");
          move_base_joint_trajectory(0.0, 0.0, 0.0, 0.5);
          break;
        }
        case 'l':
        {
        ...
        case 'm':
        {
          ...
          move_base_joint_trajectory(-1.0, +1.0, +M_PI_2+M_PI_4, 10);
          break;
        }
        case ',':
        {
          RCLCPP_DEBUG(logger, "Move Backward");
          move_base_joint_trajectory(-1.0, 0.0, +M_PI, 10);
          break;
        }
        case '.':
        {
